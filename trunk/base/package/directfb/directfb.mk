...
ifeq ($(strip $(BR2_DIRECTFB_FUSION)),y)
DIRECTFB_CONFIG += --enable-fusion
else
DIRECTFB_CONFIG += --disable-fusion
endif
ifeq ($(strip $(BR2_DIRECTFB_MULTI)),y)
DIRECTFB_CONFIG += --enable-multi
DIRECTFB_DEPENDS += fusion-kernel
else
DIRECTFB_CONFIG += --disable-multi
endif
...
$(DIRECTFB_DIR)/.configured: $(DIRECTFB_DIR)/.unpacked
	...
	./configure \
		...
		--with-inputdrivers=keyboard, linuxinput, lirc \
		--enable-jpeg \
		--enable-png \
		--enable-linux-input \
		--enable-zlib \
...
directfb: uclibc jpeg libpng freetype libsysfs $(DIRECTFB_DEPENDS) $(TARGET_DIR)/usr/lib/libdirectfb.so
