...
MEMORY
{
    FLASH (rxai) : ORIGIN = 0x00000000, LENGTH = 0x800000  /* HJB was 64M */
    INTRAM (rwxa)  : ORIGIN = 0x24000000, LENGTH = 32K
    SDRAM (rwxa) : ORIGIN = 0x10000000, LENGTH = 64M       /* HJB added */
}
...
SECTIONS
{
  ...
  /* Exception handling  */
  .eh_frame       : ONLY_IF_RW { KEEP (*(.eh_frame)) } >SDRAM AT>FLASH
  .gcc_except_table   : ONLY_IF_RW { KEEP (*(.gcc_except_table)) *(.gcc_except_table.*) } >SDRAM AT>FLASH

  /* Thread Local Storage sections  */
  .tdata	  : { *(.tdata .tdata.* .gnu.linkonce.td.*) } >SDRAM AT>FLASH
  .tbss		  : { *(.tbss .tbss.* .gnu.linkonce.tb.*) *(.tcommon) } >SDRAM AT>FLASH
  ...
  .preinit_array     : { KEEP (*(.preinit_array)) } >SDRAM AT>FLASH
  PROVIDE (__preinit_array_end = .);
  PROVIDE (__init_array_start = .);
  .init_array     : { KEEP (*(.init_array)) } >SDRAM AT>FLASH
  PROVIDE (__init_array_end = .);
  PROVIDE (__fini_array_start = .);
  .fini_array     : { KEEP (*(.fini_array)) } >SDRAM AT>FLASH
  ...
  .jcr            : { KEEP (*(.jcr)) } >SDRAM AT>FLASH
  .data.rel.ro : { *(.data.rel.ro.local) *(.data.rel.ro*) } >SDRAM AT>FLASH
  .dynamic        : { *(.dynamic) } >SDRAM AT>FLASH
  .got            : { *(.got.plt) *(.got) } >SDRAM AT>FLASH
  ...
  .data1          : { *(.data1) } >SDRAM AT>FLASH

  .balign	: { . = ALIGN(8); _edata = .; } >SDRAM AT>FLASH
  ...
  .stack         ORIGIN(SDRAM) + LENGTH(SDRAM) - 0x1000 :
  {
    _stack = .;
    *(.stack)
    . = 0x1000;
  ...
  .sram  : /* HJB section for stack at power on added */
  {
    _startup_stack = . + LENGTH(INTRAM)-4;
  } >INTRAM
